...
class ConvBlock(nn.Module):
    ...
    def forward(self,x):
        out = self.conv2(self.activiation(self.conv1(self.norm(x))))

        if self.residual:
            out = out + self.residual_conv(x)
        
        return self.activiation(out)
...
class Unet(nn.Module):
    ...
    def __init__(self, n_steps, block_channel_multiplier = 4, num_blocks = 3, pe_dim = 32, residual = True):
        ...
        for channel, cH, cW in zip(channels[0:-1], Hs[0:-1], Ws[0:-1]):
            ...
            self.downscales.append(nn.Conv2d(channel, channel, kernel_size = 3, stride = 2))
        ...
        for channel, cH, cW in zip(channels[-2::-1], Hs[-2::-1], Ws[-2::-1]):
            ...
            self.upscales.append(nn.ConvTranspose2d(prev_channel, prev_channel, kernel_size = 4, stride = 2))
            ...
            prev_channel  = channel 
        self.conv_out = nn.Conv2d(prev_channel, C, kernel_size = 3, padding = 1)
